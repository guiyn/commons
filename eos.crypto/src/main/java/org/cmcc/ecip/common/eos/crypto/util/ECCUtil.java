package org.cmcc.ecip.common.eos.crypto.util;
...
//    
	...
	public static byte[] publicEncrypt(byte[] content, PublicKey publicKey) throws Exception {
		...
		iesCipher.engineInit(Cipher.ENCRYPT_MODE, publicKey, iesParameterSpec, local.get());
	...
	public static byte[] privateDecrypt(byte[] content, PrivateKey privateKey) throws Exception {
		...
		iesCipher.engineInit(Cipher.DECRYPT_MODE, privateKey, iesParameterSpec, local.get());
		return iesCipher.engineDoFinal(content, 0, content.length);
	}
}